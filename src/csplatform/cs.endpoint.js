...
function simulateTask(req, res, next) {
    var taskId = req.params.id,
        ...
        ENTITIE_NAMES = ['Microsoft', 'Google', 'Apple', 'Grapefruit Inc.', 'Number Cruncher Ltd.', 'Compuserv Corp.', 'Banana Trade Inc.', 'Borland'],
    	isSuccess = true;
    ...
    function entity() {
        return ENTITIE_NAMES[Math.floor(Math.random() * ENTITIE_NAMES.length)];
    };
    ...
    TaskParameter.findAll({ where : { taskId : taskId}}).success(function(taskPars) {
        taskPars.forEach(function(taskPar) {
            ...
            for(var i = 0; i < 10; i++) {
                ...
                var value = (taskPar.type == "numeric") ? score() : entityName;

                taskPar.updateAttributes({
                    value: value,
                    worker : userId()
                }).success(onSuccess(deferred)).error(onError(deferred));
            }
        });
